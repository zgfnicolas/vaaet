# Operación PostgreSQL compartida — VAAET Persistence 0.1.0
...
| --- | --- |
| Endpoint | `VAAET_DB_HOST`, `VAAET_DB_PORT`, `VAAET_DB_NAME`, `VAAET_DB_SSLMODE`, `VAAET_DB_SSLROOTCERT` o `VAAET_DB_SSLROOTCERT_PEM`, `VAAET_DB_CONNECT_TIMEOUT` |
| Pool por proceso | `VAAET_DB_POOL_SIZE`, `VAAET_DB_MAX_OVERFLOW`, `VAAET_DB_POOL_RECYCLE_SECONDS` |
...
Los valores predeterminados son `pool_size=2`, `max_overflow=0` y reciclado a
300 segundos. Cada proceso mantiene su propio pool: no se comparte un engine
...
Las revisiones `0001`, `0002` y `0003` se distribuyen una sola vez dentro del
paquete. Conservan contenido, identificadores y `public.alembic_version`; mover
el código no crea `0004` ni vuelve a aplicar una base que ya está en `0003`.
...
- migración limpia y reconocimiento de una base existente en `0003`;
...
Reporta revisión, TLS y rol activo, owners de schemas, constraints pendientes,
cobertura de comentarios, índices, tamaños, autovacuum, integridad y `EXPLAIN`
...
restaura DDL o roles sobre una base viva. Ante una adopción fallida de la nueva
biblioteca, revertí el código: esta entrega no cambia esquema ni datos.
